#!/usr/bin/env python3
"""Compute motor balance and gyro noise metrics from a decoded Blackbox log.

AGENT.md and CLAUDE.md both name these two metrics as what a flight log entry
should record, but nothing computed them -- `capture_log.py` emitted `0.00`
placeholders. This closes that gap.
...
Metrics:
  ...
  * Gyro noise floor -- mean spectral magnitude above 100 Hz, per axis. Below
    that, motion dominates; above it, what remains is frame and motor noise.

`numpy` is used when present and a pure-Python FFT is used otherwise, so the
project's pyserial-only install stays valid.
"""
import argparse
import cmath
import csv
